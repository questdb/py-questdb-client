...
class TestSchemaOverridesPandasManual(unittest.TestCase):
    """Pure NumPy-backed pandas + schema_overrides (ipv4 / char / geohash)
    routes through the manual columnar planner and does not import pyarrow.
    Arrow-backed pandas keeps using the capsule path; that split is covered
    by `TestPandasPlannerRouting` / `TestSchemaOverridesPandas`."""

    def _numpy_ts(self, n):
        import numpy as np
    ...
    def _ingest(self, df, table_name, overrides):
        pa_before = qi._debug_dataframe_pyarrow_loaded()
        with QwpAckServer() as server:
            client = qi.Client.from_conf(_client_conf(server.port))
            try:
                client.dataframe(
                    df, table_name=table_name, at='ts',
        ...
        if not pa_before:
            self.assertFalse(
                qi._debug_dataframe_pyarrow_loaded(),
                'pandas-native + schema_overrides must not import pyarrow')

    def test_ipv4_uint32_manual(self):
        import pandas as pd
        df = pd.DataFrame({
            'addr': pd.Series([0x0A000001, 0xC0A80101], dtype='uint32'),
            'ts': pd.Series(self._numpy_ts(2)),
        })
        self._ingest(df, 'ipv4_numpy', {'addr': 'ipv4'})

    def test_char_uint16_manual(self):
        import pandas as pd
        df = pd.DataFrame({
            'c': pd.Series([65, 66, 67], dtype='uint16'),
            'ts': pd.Series(self._numpy_ts(3)),
        })
        self._ingest(df, 'char_numpy', {'c': 'char'})

    def test_geohash_int32_manual(self):
        import pandas as pd
        df = pd.DataFrame({
            'g': pd.Series([12345, 67890], dtype='int32'),
            'ts': pd.Series(self._numpy_ts(2)),
        })
        self._ingest(df, 'geohash_numpy', {'g': ('geohash', 20)})

    def test_ipv4_on_non_uint32_rejected(self):
        import pandas as pd
        df = pd.DataFrame({
            'addr': pd.Series([1, 2], dtype='int64'),
            'ts': pd.Series(self._numpy_ts(2)),
        })
        with QwpAckServer() as server:
            client = qi.Client.from_conf(_client_conf(server.port))
    ...
    def test_geohash_on_unsigned_rejected(self):
        import pandas as pd
        df = pd.DataFrame({
            'g': pd.Series([1, 2], dtype='uint32'),
            'ts': pd.Series(self._numpy_ts(2)),
        })
        with QwpAckServer() as server:
            client = qi.Client.from_conf(_client_conf(server.port))
    ...
    @unittest.skipIf(pa is None, 'pyarrow not installed')
    def test_arrow_backed_numeric_override_in_mixed_manual_frame(self):
        # An object column forces the manual planner; an Arrow-backed uint32
        # column in the same frame carries the ipv4 override and is emitted
        # through the Rust Arrow import override branch.
        import pandas as pd
        df = pd.DataFrame({
            'name': pd.Series(['a', 'b'], dtype='object'),
            'addr': pd.array(
                pa.array([0x0A000001, 0xC0A8010A], type=pa.uint32()),
        ...
        with QwpAckServer() as server:
            ...
            try:
                client.dataframe(
                    df, table_name='mixed_ipv4', at='ts',
                    schema_overrides={'addr': 'ipv4'})
            finally:
                client.close()
            stats = server.snapshot()
    ...
    def test_geohash_bits_exceed_column_width_rejected(self):
        # 40 bits cannot fit an int16 (max 16) column; rejected before a
        # connection is borrowed.
        import pandas as pd
        df = pd.DataFrame({
            'g': pd.Series([1, 2], dtype='int16'),
            'ts': pd.Series(self._numpy_ts(2)),
        })
        with QwpAckServer() as server:
            client = qi.Client.from_conf(_client_conf(server.port))
