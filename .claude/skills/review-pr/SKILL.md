...
- **Judge concurrency against the threading contract.** The `QuestDB` handle is thread-safe. `Sender`, `Buffer`, `PooledSender` and `QueryResult` are used by one thread at a time and may be handed between threads with synchronization; a `PooledReader` stays on the thread that borrowed it. Every `PooledSender`/`PooledReader` method holds the lease's lock for its whole call, so a second thread waits rather than interleaving; a new method that releases that lock mid-call breaks the contract and is a finding. Two threads calling into one of those objects at once is otherwise user error: report it only if it crashes the interpreter, corrupts memory, or loses rows without an exception or a `questdb` log record. A refusal, a wait, or an imprecise message on that path is at most Minor. Same-thread re-entry from the caller's own Python (a column value's conversion, an `attrs` getter, an Arrow producer, a callback) is inside the contract and is reviewed in full.
...
- **Cython exception convention:** does the function return a C type with the right `except` clause? In Cython 3, a non-`extern` `cdef`/`cpdef` function with no explicit clause gets a propagating exception specification appropriate to its return type (for example, `except? -1` for `int` and `except *` for `void`); `nogil` does not change that default. Exceptions are swallowed only when propagation is disabled, such as by explicit `noexcept`, an implicit-`noexcept` `cdef extern` declaration, or the legacy `legacy_implicit_noexcept=True` directive. Did the convention change, and do all callers still propagate errors correctly? For `nogil` functions, also check whether an implicit `except *` forces a GIL reacquisition at every call.
...
- **Cython exception-default fact:** in Cython 3, a non-`extern` `cdef`/`cpdef` function without an explicit exception clause defaults to a propagating specification appropriate to its return type (for example, `except? -1` for `int` and `except *` for `void`), including when declared `nogil`. A `cdef extern` declaration instead defaults to `noexcept`; explicit `noexcept` and `legacy_implicit_noexcept=True` also disable propagation. Agents 1, 2, and 3 must determine the effective specification on every changed `cdef`, distinguish internal definitions from external declarations, and check whether a `nogil` function's implicit `except *` introduces per-call GIL reacquisition.
...
- **Silent exception swallowing:** a Cython-implemented function declared `noexcept` (explicitly, or through the legacy `legacy_implicit_noexcept=True` directive), or an `extern` declaration left implicitly `noexcept` even though the external function can set a Python exception, drops that exception on the floor. An omitted clause on a non-`extern` `cdef`/`cpdef` propagates by default in Cython 3. Verify the effective exception convention against what the body or external function can raise.
...
6. **For exception-swallowing claims:** check whether the function is an internal definition or an `extern` declaration, determine its effective exception specification, and verify whether the body or external function can raise. Under Cython 3, omitting the clause on a non-`extern` `cdef`/`cpdef` propagates exceptions by default even with `nogil`; implicit `noexcept` applies to `cdef extern`, while explicit `noexcept` and `legacy_implicit_noexcept=True` also disable propagation.
...
- Correct Cython exception convention on every `cdef`/`cpdef` returning a C type (non-`extern` functions propagate by default in Cython 3, including with `nogil`; `cdef extern`, explicit `noexcept`, and `legacy_implicit_noexcept=True` require scrutiny for swallowed exceptions)
