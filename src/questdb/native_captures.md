...
| 13 | `Buffer._column` | `c_name` — `line_sender_column_name` borrowed from the arena | No `_column_*` helper accepts a `line_sender_column_name`: each takes the name as a `str` and encodes it inside itself, immediately before its own native call and after any conversion that runs caller Python. The borrow cannot span caller code because it never crosses a function boundary. `test_no_column_helper_takes_a_pre_encoded_name` holds the signatures there | ownership by construction |
